...
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap">
    ...
    <div class="d-flex align-items-center">
        {{-- フィルター全体を一つのフォームで囲む --}}
        <form id="filter-form" action="{{ route('dashboard') }}" method="GET" class="d-flex align-items-center flex-wrap">
            <div class="btn-group me-2 mb-2 mb-sm-0">
                <a href="{{ route('dashboard', array_merge(request()->except('range', 'start_date', 'end_date'), ['range' => 'today'])) }}" class="btn {{ $currentRange == 'today' ? 'btn-primary' : 'btn-outline-secondary' }}">今日</a>
                <a href="{{ route('dashboard', array_merge(request()->except('range', 'start_date', 'end_date'), ['range' => 'month'])) }}" class="btn {{ $currentRange == 'month' ? 'btn-primary' : 'btn-outline-secondary' }}">今月</a>
                <a href="{{ route('dashboard', array_merge(request()->except('range', 'start_date', 'end_date'), ['range' => 'all'])) }}" class="btn {{ $currentRange == 'all' ? 'btn-primary' : 'btn-outline-secondary' }}">今年</a>
            </div>

            {{-- 日付範囲ピッカー (カスタム用) --}}
            <div class="d-flex me-2 mb-2 mb-sm-0">
                <input type="hidden" name="range" value="custom">
                {{-- 既存のクエリパラメータ（store_idなど）を維持するためのhidden input --}}
                @if($selectedStoreId)
            ...
            <div class="mb-2 mb-sm-0">
                <select name="store_id" class="form-select" onchange="document.getElementById('filter-form').submit()">
